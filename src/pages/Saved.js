...
const SaveBtn=({data})=>{
  ...
  return(<button
  onClick={(e)=>handleClick(e)} className="outline-0 border-0 bg-none cursor-pointer mx-2">
  <svg
...
const Saved = () => {
  ...
  return (
    <section className="w-[80%] h-full flex flex-col mt-16 mb-24 relative">
      <div className=" w-full min-h-[60vh]  border border-gray-100  rounded  py-8">
        {savedData ? (
...
<table className="w-full table-auto">
  ...
  <tr
  key={data.id}
                    className="text-center text-base border-b border-gray-100 hover:bg-purple hover:scale-110 last:border-b-0"
                  >
                    <td className="py-4 flex items-center uppercase  ">
           <SaveBtn data={data}/>
                      <img
                        className="w-[1.5rem] h-[1.2rem] ml-1.5 mx-2"
                        src={data.image}
                        alt={data.name}
                      />
                      <span className="mx-2">
