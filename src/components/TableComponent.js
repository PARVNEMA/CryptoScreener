...
const SaveBtn = ({ data }) => {
  ...
  return (
    <button
      ...
      className="outline-0 border-0 bg-none cursor-pointer mx-2"
    >
      <svg
...
const TableComponent = () => {
  ...
  return (
    <>
          ...
          <table className="w-full table-auto">
            <thead className="capitalize  text-md text-gray-200 font-bold border-b border-gray-100">
              <tr>
                ...
                <th className="py-1
                lg:table-cell hidden">Market Cap Change</th>
            ...
            <tbody>
              {cryptoData.map((data) => {
                return (
                  <tr
                    key={data.id}
                    className="text-center text-md border-b border-gray-100  hover:bg-opacity-5 last:border-b-0 hover:scale-105 "
                  >
                    <td className="py-4 flex items-center uppercase mr-2  ">
                      <SaveBtn data={data} />
                      <img
                        className="w-[2.5rem] h-[2.4rem] ml-1.5 mx-2 "
                        src={data.image}
                        alt={data.name}
                      />
                      <span className="mx-2">
      ...
      <div className="md:flex md:items-center md:justify-between md:mt-4 capitalize md:h-[2rem] ">
        <span>
          Data Provided bt{" "}
          <a
            className="text-cyan"
